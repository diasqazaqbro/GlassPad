...
final class LaunchpadModel {
    ...
    /// Apps filtered + ranked by the current query (instant, fuzzy). Searches all
    /// apps, including those tucked inside folders.
    ...
    /// What the grid shows: the arrangement normally, flat search hits while typing.
    var displayedItems: [LaunchpadItem] {
        searching ? filteredApps.map(LaunchpadItem.app) : items
    }
    ...
    var pages: [[LaunchpadItem]] {
        ...
        return stride(from: 0, to: all.count, by: cap).map {
            Array(all[$0 ..< min($0 + cap, all.count)])
        }
    }
    ...
    var selectedItemID: String? {
        guard let i = selectedIndex, displayedItems.indices.contains(i) else { return nil }
        return displayedItems[i].id
    }
    ...
    func loadApps() {
        Task {
            ...
            async let storedTask = Task.detached(priority: .userInitiated) {
                LayoutStore.load()
            }.value
            let (discovered, stored) = await (discoveredTask, storedTask)
    ...
    func handleQueryChange() {
        currentPage = 0
        selectedIndex = (searching && !displayedItems.isEmpty) ? 0 : nil
    }
    ...
    func move(_ direction: MoveDirection) {
        ...
        guard let current = selectedIndex, all.indices.contains(current) else {
            selectedIndex = min(currentPage * cap, all.count - 1)
            return
        }
    ...
    func activateSelected() -> Bool {
        ...
        case .app(let app):
            ...
            return true
        case .folder(let folder):
            withAnimation(.bouncy) { openFolder = folder }
            return false
        }
    }
    ...
    func persist() {
        let layout = StoredLayout(version: LayoutStore.currentVersion, entries: items.map { item in
            switch item {
